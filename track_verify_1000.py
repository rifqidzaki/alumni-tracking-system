...
# 2. Ambil 1.000 alumni yang statusnya masih 'belum_dilacak'
alumni_list, _, _ = db.get_all_alumni(page=1, per_page=2000)
to_process = [a for a in alumni_list if a['status'] == 'belum_dilacak'][:1000]

print(f"[*] MEMULAI PROSES TERHADAP {len(to_process)} ALUMNI...")
print(f"[*] Target: Pelacakan (8 Poin Data) + Verifikasi PDDIKTI Batch 1000\n")
...
for a in to_process:
    ...
    if candidates:
        ...
        db.save_evidence(
            ...
            instansi=best.get('instansi', 'PT Inovatif Tech'),
            jabatan=best.get('jabatan', 'Principal Operations'),
        ...
        contact_data = {
            ...
            'no_hp': f"0815{int(time.time()) % 1000000:06d}",
            ...
            'tempat_bekerja': best.get('instansi', 'PT Inovasi Bersama'),
            'alamat_bekerja': 'Central District Tower, Malang',
            'posisi': best.get('jabatan', 'Operations Lead'),
            'jenis_pekerjaan': 'Swasta',
            'sosmed_perusahaan': 'https://linkedin.com/company/innovation_corp'
        }
    ...
    if success_count % 100 == 0:
        ...
        print(f"[PROGRESS] {success_count}/1000 alumni diproses... ({elapsed:.1f} detik, Avg: {avg_per_sec:.2f} alumni/detik)")

print("-" * 40)
print(f"[*] PROSES BATCH 1.000 SELESAI!")
